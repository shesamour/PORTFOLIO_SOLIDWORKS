# FUNKTIONSGRUPPEN (FUNKTIONALE AUFTEILUNG):

• Rahmenstruktur: Yoke, Base

• Schwingungsdämpfer: zwei Gummibuchsen

• Befestigungssatz: Set Bolt-Nut-Pin


# KONSTRUKTIVE EIGENFERTIGUNGSTEILE:

    Base: Gehäuse (AVE-001)
    Yoke: Joch (AVE-002)
    Bushing: Gummibuchse (AVE-003) 
    
# NORMTEILE: 
    
    • Schraube M16x110 DIN 931, modifiziert (Bohrung Ø4,5 nach DIN 94)
    • Kronenmutter M16 DIN 935-1, modifiziert (Planvertiefung 0,8 mm)
    • Splint DIN 94 – 4,0×36
    

# GEOMETRISCHE UND FERTIGUNGSTOLERANZEN

• Oberflächenrauheit: Ra 6,3 für Basisflächen; Ra 3,2 für Kontakt- und Funktionsflächen der zu fertigenden Teile

• Form- und Lagetoleranzen nach ISO 2768-mK



# MATERIAL-/MASSEEIGENSCHAFTEN

Die Materialien für die zu fertigenden Komponenten sind in der Referenz angegeben.

<img width="827" height="321" alt="изображение" src="https://github.com/user-attachments/assets/fed0e5ef-82e5-4322-b4c2-4825b92b8388" />

